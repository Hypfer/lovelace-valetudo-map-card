class ValetudoMapCard extends HTMLElement {
  ...
  drawMap(mapLegacyMode, mapContainer, attributes, mapHeight, mapWidth, floorColor, wallColor, currentlyCleanedZoneColor, noGoAreaColor, virtualWallColor, pathColor, chargerColor, vacuumColor, gotoTargetColor) {
    // Points to pixels
    let pixelSize = 50;
    if (!mapLegacyMode) {
    ...
    let mapTopOffset = 0;
    if (mapLegacyMode) {
      objectLeftOffset = attributes.image.position.left * this._config.map_scale;
      objectTopOffset = attributes.image.position.top * this._config.map_scale;
    } else {
    ...
    let chargerInfo = this.getChargerInfo(attributes, mapLegacyMode);
    ...
    let robotInfo = this.getRobotInfo(attributes, mapLegacyMode);
    if(!robotInfo) {
    ...
    let goToInfo = this.getGoToInfo(attributes, mapLegacyMode);
    ...
    if (this._config.show_floor) {
      ...
      mapCtx.beginPath();
      let floorPoints = this.getFloorPoints(attributes, mapLegacyMode);
      if (floorPoints) {
    ...
    let segmentAreas = this.getSegments(attributes, mapLegacyMode);
    ...
    if (this._config.show_walls) {
      ...
      mapCtx.beginPath();
      let wallPoints = this.getWallPoints(attributes, mapLegacyMode);
      if (wallPoints) {
    ...
    if (attributes.currently_cleaned_zones && this._config.show_currently_cleaned_zones) {
    ...
    let noGoAreas = this.getNoGoAreas(attributes, mapLegacyMode);
    ...
    let virtualWallPoints = this.getVirtualWallPoints(attributes, mapLegacyMode);
    ...
    let pathPoints = this.getPathPoints(attributes, mapLegacyMode);
    if (pathPoints) {
    ...
    let predictedPathPoints = this.getPredictedPathPoints(attributes, mapLegacyMode);
  ...
  set hass(hass) {
    ...
    let attributes = mapEntity.attributes;

    if (mapEntity && mapEntity['state'] !== 'unavailable' && attributes) {
      if(typeof attributes.deflatedMapAsBase64 === "string") {
        try {
          attributes = JSON.parse(pako.inflate(atob(attributes.deflatedMapAsBase64), { to: 'string' }));
        } catch (e) {
          //Help how do I handle errors in lovelace cards?
        }
      }
    ...
    if (canDrawMap) {
      ...
      let height;
      if (mapLegacyMode) {
        width = attributes.image.dimensions.width;
        height = attributes.image.dimensions.height;
      } else {
        let floorLayer = this.getLayers(attributes, 'floor', 1)[0];
      ...
      if (this.shouldDrawMap(mapEntity, hass.selectedTheme)) {
        ...
        this.drawMap(mapLegacyMode, this.mapContainer, attributes, mapHeight, mapWidth, floorColor, wallColor, currentlyCleanedZoneColor, noGoAreaColor, virtualWallColor, pathColor, chargerColor, vacuumColor, gotoTargetColor);
